...
import { LESSION_ROUTES } from "@/constants/routes";

// --- MOCK DATA: Lesson Slides ---
// In a real app, you would fetch this based on params.levelId
const lessonSlides = [
  {
    id: 1,
    type: "intro",
    ...
    content:
      "We count in Base 10 every day because we have 10 fingers. But computers only have 'switches' (on/off), so they count in Base 2 (Binary).",
    emoji: "🖐️",
  },
  {
    id: 2,
    ...
    content:
      "A 'Base' tells you how many digits you can use before you have to add a new column. \n\n• Base 10: 0-9 \n• Base 2: 0-1 \n• Base 8: 0-7",
    emoji: "🧮",
  },
  {
    id: 3,
    ...
    content:
      "In Base 2 (Binary), the number '5' is written as '101'. \n\n1 (fours place) \n0 (twos place) \n1 (ones place) \n\n4 + 0 + 1 = 5!",
    emoji: "💡",
  },
  {
    id: 4,
...
export default function LevelPage({
  ...
  const handleNext = () => {
    if (currentSlide < totalSlides - 1) {
      ...
      router.push(
        LESSION_ROUTES.QUIZ(params.courseId, params.unitId, params.levelId)
      );
    }
  };
      ...
      <header className="flex items-center gap-4 p-6 w-full z-20 bg-white/90 backdrop-blur-sm sticky top-0">
        ...
        <div className="flex-1 flex gap-2 h-2">
          {lessonSlides.map((_, index) => (
            <div
              key={index}
              className={`h-full rounded-full flex-1 transition-all duration-300 ${ index <= currentSlide ? "bg-[#4854F6]" : "bg-gray-200" }`}
            />
          ))}
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6">
              <span className="text-4xl">
                {lessonSlides[currentSlide].emoji}
              </span>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className={`
              ...
              ${ currentSlide === 0 ? "bg-gray-100 text-gray-300 cursor-not-allowed" : "bg-gray-200 text-gray-500 hover:bg-gray-300 btn-press" }
            `}
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <button
            ...
            className={`
              ...
              ${ currentSlide === totalSlides - 1 ? "bg-green-500 text-white shadow-[0_4px_0_#16a34a]" : "bg-[#4854F6] text-white shadow-[0_4px_0_#353EB5]" }
            `}
          >
