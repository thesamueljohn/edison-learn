...
export const NavItem = ({ icon, label, isActive, onClick, badge, href }) => {
  return (
    <Link href={href || "#"}>
      <button
        onClick={onClick}
      ...
      >
        <div className="relative">
          {icon}
          {badge && (
            <span className="absolute -top-2 -right-3 bg-red-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-md">
              {badge}
            </span>
          )}
        </div>
...
export const MobileNavItem = ({ icon, label, isActive, onClick, href }) => (
...
export const QuestItem = ({ title, progress, total, icon }) => (
...
export const Target = ({ size, className }) => (
  <svg
